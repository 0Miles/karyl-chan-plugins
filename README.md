...
Bot 設定了 `WEB_BASE_URL` 之後，每個 Plugin 的 HTTP surface 都透過
`<WEB_BASE_URL>/plugin/<pluginKey>/` 代理給瀏覽器存取，不需要 Plugin 自己開放
公開 port 或申請 TLS 憑證。SDK 在 register / heartbeat 回應中接收 `publicBaseUrl`
並透過 `StartedPlugin.getPublicBaseUrl()` 和 `CommandContext.publicBaseUrl` 暴露。
Radio Plugin 的 WebUI 透過 `<bot>/plugin/karyl-radio/` 存取即可。
