...
// ── v1 API（廢棄，保留以不破壞現有 plugin build）───────────────────────────

/**
 * @deprecated v1 CommandDefinition。v2 請改用 PluginCommandDefinition + definePluginCommand。
 * 型別保留，export 保留；M1-E 升級後移除。
 */
...
/**
 * @deprecated v1 PluginConfig。v2 請改用 PluginConfigV2 + definePlugin。
 * 型別保留，export 保留；M1-E 升級後移除。
 */
export interface PluginConfig {
  /** Plugin identifier — used as manifest.plugin.id and in URL defaults. */
  key: string;
  name: string;
  version: string;
  description?: string;
   ...
   * `server.listen()`. Use this to register additional routes or
   * Fastify plugins on the server instance — Fastify locks routing
   * after listen, so any `server.get(...)` etc. must happen here.
   * May be sync or async; SDK awaits the returned promise before
   * calling listen.
   */
...
/**
 * @deprecated v1 API。v2 請改用 definePluginCommand。
 *
 * 保留 export 以避免既有 v1 plugin 在 import 時立即爆炸（import error vs runtime error）。
 * 呼叫此函式會在 runtime 拋出錯誤，引導作者升級。
 * M1-E 升級後移除此 export。
 *
 * @throws Error 總是拋出，提示作者升級到 v2。
 */
export function defineCommand(_def: CommandDefinition): CommandDefinition {
  throw new Error(
    "defineCommand removed in SDK v2, use definePluginCommand instead. " +
      "See the Plugin SDK v1→v2 Migration Guide.",
  );
}
...
/**
 * v2 Plugin config，傳入 definePlugin。
 */
...
export interface PluginInstance {
  readonly config: PluginConfigV2 | PluginConfig;
 ...
 * 定義一條 plugin 自訂指令（軌三）。
 * 回傳定義物件不變（類型化建構子）。
 * 取代 v1 defineCommand（後者已 @deprecated）。
 */
 ...
 *
 * Accepts PluginConfigV2（推薦）或 PluginConfig（v1，@deprecated，M1-E 升級後移除）。
 */
export function definePlugin(config: PluginConfigV2): PluginInstance;
/** @deprecated 請改用 PluginConfigV2。M1-E 升級後移除此 overload。 */
export function definePlugin(config: PluginConfig): PluginInstance;
export function definePlugin(
  config: PluginConfigV2 | PluginConfig,
): PluginInstance {
  if (isPluginConfigV2(config)) {
  ...
  return {
    ...
    async start(opts?: StartOptions): Promise<StartedPlugin> {
      ...
      const { createPluginServer } = await import("./server.js");
      ...
      if (isPluginConfigV2(config)) {
        ...
        if (setupSecret && setupSecret.trim().length > 0) {
          const { buildManifestV2 } = await import("./manifest-builder.js");
          const manifest = buildManifestV2(config, pluginUrl);
        ...
        const { callBotRpc } = await import("./server.js");
        ...
        // ── v1 deprecated path ────────────────────────────────────────────
        const v1Config = config as PluginConfig;
        const server = createPluginServer({
          pluginKey: v1Config.key,
          botUrl,
        ...
        if (setupSecret && setupSecret.trim().length > 0) {
          ...
          const manifest = buildManifest({
            ...
            rpcMethodsUsed: v1Config.rpcMethodsUsed,
            storage: v1Config.storage,
            commands: v1Config.commands.map((cmd) => ({
              name: cmd.name,
